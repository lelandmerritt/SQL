...
SELECT WINNer, Max(winner_pts) AS Average_winning_score FROM superbowls GROUP BY winner;

SELECT loser, MAX(loser_pts) AS AVERAGE_losing_score FROM superbowls GROUP BY loser HAVING average_losing_score > 50;

SELECT COUNT(*),
    CASE
        WHEN date > 1996 THEN "Pre Jaguars"
        WHEN date > 1995 THEN "pre golden era"
        ELSE "Prehistoric"
        END as "eras"
        FROM superbowls
        GROUP BY eras;

SELECT * FROM superbowls WHERE winner = "New England Patriots" AND MVP = "Tom Brady";
